...
function parseExternalOutput(rule, stdout, meta = {}) {
  ...
  if (adapter === "sarif")
    return annotateRuleViolations(rule, parseSarifOutput(rule, stdout, meta));
...
function ruleViolationMessage(violation) {
  const location = `${violation.file}:${violation.line}${violation.column ? `:${violation.column}` : ""}`;
  return `RULE [${violation.rule}]${ruleReference(violation)} ${location} ${violation.message}`;
}
